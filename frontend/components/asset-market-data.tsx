...
import type { Asset, MarketBar, MarketQuote } from "@/lib/types";
  ...
  return (
    <>
      <header className="asset-header">
        ...
        <div className="asset-quote">{quote ? <><strong>{currency.format(Number(quote.close))}</strong><span className={change === null ? "" : change >= 0 ? "positive" : "negative"}>{change === null ? "Нет предыдущего закрытия" : `${change >= 0 ? "+" : ""}${change.toFixed(2)}% к предыдущему закрытию`}</span></> : <strong className="missing-value">Нет данных</strong>}</div>
      </header>
