...
export default function PortfolioPage() {
  return (
    <>
      <PageHeader title="Анализ портфеля" description="Аналитика активов, которыми пользователь фактически владеет и указывает вручную." action={<span className="timestamp">Данные рынка: 12:00 UTC</span>} />
      ...
      <section className="panel news-impact-panel"><div className="panel-heading"><div><p className="eyebrow">Информационные факторы</p><h2>Влияние последних новостей на активы портфеля</h2></div><span className="timestamp">Обновлено 09:30 UTC</span></div><div className="news-impact-list"><article><span className="impact negative-news">Негативное</span><div><strong>Technology sector policy review</strong><p>Политическая неопределённость повышает риск-премию позиций AAPL и MSFT.</p></div><div className="asset-pills"><span>AAPL</span><span>MSFT</span></div></article><article><span className="impact positive-news">Позитивное</span><div><strong>Long-term yields stabilize</strong><p>Стабилизация доходностей поддерживает оценку позиции TLT.</p></div><div className="asset-pills"><span>TLT</span></div></article></div></section>
    </>
  );
}
